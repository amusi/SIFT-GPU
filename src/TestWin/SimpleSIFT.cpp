...
int main()
{
    ...
    #ifdef _WIN32
        #ifdef _DEBUG
            HMODULE  hsiftgpu = LoadLibrary("SIFTGPU_d.dll");	// Amusi mod: siftgpu.dll
        #else
	...
	{
		std::cerr << "SIFTGPU not FULL_SUPPORTED! ! !" << std::endl;
		return 0;
	}

	//[2018-04-25] Amusi mod: 修改路径，因为工程下测试和直接用可执行程序测试的路径不一样
    if(sift->RunSIFT("F:/Projects/Stitch/C++/GPU-SIFT/SiftGPU/data/800-1.jpg"))
    {
    ...
    if(sift->RunSIFT("F:/Projects/Stitch/C++/GPU-SIFT/SiftGPU/data/640-1.jpg"))
    {
